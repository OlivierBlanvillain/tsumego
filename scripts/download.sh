...
for collection in 186 187 188 190 193 194; do
  ...
  (cd "$collection"; ls *.sgf | while read p; do
    cat "$p" | sgf $p -c | sgfstrip AN AP BR BT CP DT EV FF GM GN HA KM ON OT PB PC PL PW RE RO RU SO SZ TM US WR WT GE DI DP CO CA ST TR MA > "../tmp/problem-$collection-$p"
  done)
